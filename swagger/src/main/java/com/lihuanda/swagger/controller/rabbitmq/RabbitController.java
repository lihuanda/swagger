 ...
 */
@RestController
@Api(value="SimpleRabbitDemo", tags="SimpleRabbitDemo")
public class RabbitController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "message", dataType = "string", required = false, value = "信息"),
    })
    @ApiOperation(value="Rabbit发送普通字符串", notes="字符串", httpMethod = "GET")
     ...
     * @return
     */
    @ApiOperation(value="RabbitMQ发送对象", notes="RabbitMQ发送对象", httpMethod = "POST")
    @RequestMapping(value = Prefix+"/object/message/send",method = RequestMethod.POST,consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public BaseResponse sendObjectMessage(@RequestBody(required = false) User1 user){
        BaseResponse response=new BaseResponse(StatusCode.Success);
        try {
            ...
            rabbitTemplate.convertAndSend(user, new MessagePostProcessor() {
                ...
                public Message postProcessMessage(Message message) throws AmqpException {
                    ...
                    messageProperties.setHeader(AbstractJavaTypeMapper.DEFAULT_CONTENT_CLASSID_FIELD_NAME,User1.class);
                    return message;
                }
            });
